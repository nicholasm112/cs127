...
def average(region):
     ...
     """
     #x = p.getRed() cant use get color becuase colors arent alway exactly the same at every pixel
   
     r = np.average(region[:,:,0])
     g = np.average(region[:,:,1])
     b = np.average(region[:,:,2])
...
def setRegion(region, r,g,b):
     ...
     region[:,:,1] = g
     region[:,:,2] = b
     # or u coudlve done region[:,:,0] = r then region[:,:,1] = b ...
     ###################################
     ### FILL IN YOUR CODE HERE      ###
     ### Other than your name above, ###
     ### this is the only section    ###
